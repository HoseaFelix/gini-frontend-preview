...
const UploadSequence = ({ initialStep = 0, initialResume = null }: UploadSequenceProps) => {
    const totalItems = 4
    const [currentView, setCurrentView] = useState(initialStep)
    const [document, setDocument] = useState<File | null>(null)
    /* eslint-disable-next-line @typescript-eslint/no-unused-vars */
    const [canContinue, setCanContinue] = useState(initialStep > 0)
    ...
    useEffect(() => {
        if (initialResume) {
            setOriginalResume(initialResume);
        }
    }, [initialResume, setOriginalResume]);
    ...
    const handleFirstContinue = async () => {
        ...
        try {
            ...
            if (analyzeDoc?.success && analyzeDoc?.parsedResume) {
                ...
                try { void createActivity('Upload Resume', document?.name ?? 'uploaded file') } catch { }
    ...
    const handleLastContinue = () => {
        ...
        try { void createActivity('Apply Template', `Template ${templateIndex != null ? templateIndex + 1 : 'unknown'}`) } catch { }
                                    ...
                                    <div className='flex flex-col overflow-y-auto md:flex-row gap-5 sm:gap-10 mt-10 sm:mt-16'>
                                        <div className='relative min-w-0'>
                                            ...
                                            <label htmlFor="fileUpload" className='w-full'>
                                                <div className='overflow-hidden max-sm:w-[200px]  max-sm:h-[150px] py-5 w-[250px] h-[250px]  border rounded-lg border-text/50 flex items-center justify-center hover:cursor-pointer flex-col gap-5 pt-7 px-4'>
                                                    {document && (
                                                        <div className='mx-auto flex items-center w-full h-full overflow-hidden text-center justify-center'>
                                                            <span className='truncate w-full px-2' title={document.name}>{document.name}</span>
                                                        </div>
                                                    )}
